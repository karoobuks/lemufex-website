...
// const [globalEnrollmentDate, setGlobalEnrollmentDate] = useState(null);
...
//       // ✅ Save overall enrollment date (from trainee.createdAt)
//       setGlobalEnrollmentDate(data.data.enrollmentDate);

//       // ✅ Build training-specific enrollment dates map
//       const dates = {};
//       data.data?.trainings?.forEach(training => {
//         if (training.courseName && training.enrolledAt) {
//           dates[training.courseName] = training.enrolledAt;
//         }
//       });
...
//                       Enrolled:{" "}
//                       {enrollmentDates[training.track]
//                         ? new Date(enrollmentDates[training.track]).toLocaleDateString()
//                         : globalEnrollmentDate
//                         ? new Date(globalEnrollmentDate).toLocaleDateString()
//                         : "N/A"}
//                     </p>
...
function ResourcesCard({ user }) {
  ...
  if (user?.role === "admin")
    return <DashboardCard icon={icon} title={title} href="/dashboard/resources" desc={desc} />;

  if (user?.isTrainee) {
    const tracks = user.tracks || [];
    if (tracks.length === 0)
      return <DashboardCard icon={icon} title={title} href="/register-training" desc="Enroll in a training program to unlock resources." />;
    if (tracks.length === 1)
      return <DashboardCard icon={icon} title={title} href={`/dashboard/resources/${slugify(tracks[0])}`} desc={desc} />;
...
export default function TraineeDashboard() {
  ...
  const fetchEnrollmentDates = async () => {
    try {
      ...
      const dates = {};
      data.data.trainings.forEach((t) => {
        if (t.track && t.enrolledAt) dates[t.track] = t.enrolledAt;
      });
      setEnrollmentDates(dates);
    } catch (error) {
      console.error("Error fetching enrollment dates:", error);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-6">
        ...
        {/* ✅ Dashboard Cards */}
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          <DashboardCard icon={<FaBookOpen />} title="My Trainings" href="/dashboard/my-trainings" desc="Access your course materials and modules" />
          <DashboardCard icon={<FaCalendarAlt />} title="My Schedule" href="/dashboard/schedule" desc="View upcoming classes and deadlines" />
          <DashboardCard icon={<FaChartLine />} title="Progress Tracker" href={`/dashboard/${userId}/progress`} desc="Monitor your learning progress" />
          <ResourcesCard user={userForResources} />
          <DashboardCard icon={<FaUserCircle />} title="My Profile" href="/dashboard/profile" desc="Update your personal information" />
          <DashboardCard icon={<FaPlusCircle />} title="Enroll More Programs" href="/dashboard/enroll-more" desc="Explore additional engineering courses" />
        </div>
      </div>
    </div>
  );
