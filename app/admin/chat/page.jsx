...
//                   className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE9900] focus:border-transparent text-gray-900 placeholder:text-gray-700 placeholder:font-semibold"
//                   disabled={false}
//                 />
//                 <button
//                   type="submit"
...
export default function AdminChatPage() {
  ...
  useEffect(() => {
    if (session?.user?.role === 'admin') {
      ...
      const initSocket = () => {
        ...
        socketRef.current = io(url, { transports: ['websocket','polling'], timeout: 5000 });

        socketRef.current.on('connect', () => {
          console.log('Admin socket connected');
          
          // Emit admin connection info for status tracking
          socketRef.current.emit('user-connect', {
            userId: session.user.id,
            role: 'admin',
            name: session.user.firstName || 'Admin'
          });
          
          socketRef.current.emit('join-admin');
          if (selectedChat) socketRef.current.emit('join-chat', selectedChat._id);
        });

        socketRef.current.on('admin-notification', (data) => {
          try { new Audio('/notification.mp3').play().catch(()=>{}); } catch(e){}
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('Lemufex Admin', { body: `New message from ${data.userName}`, icon: '/favicon.ico' });
          }
          toast.success(`📨 New message from ${data.userName}`, {
            duration: 5000,
            style: { background: '#F59E0B', color: 'white' }
          });
          setChatUnreadCounts(prev => ({ ...prev, [data.chatId]: (prev[data.chatId] || 0) + 1 }));
          loadChats();
        });
        ...
        socketRef.current.on('new-message', (message) => {
          if (selectedChat?._id === message.chatId && message.senderId !== session.user.id) {
            ...
            setChatUnreadCounts(prev => ({ ...prev, [message.chatId]: (prev[message.chatId] || 0) + 1 }));
          }
        });

        socketRef.current.on('user-typing', (data) => {
          if (selectedChat?._id === data.chatId && data.userId !== session.user.id) {
            setOtherUserTyping(data.isTyping ? { userName: data.userName || 'User', userId: data.userId } : null);
          }
        });

        socketRef.current.on('user-online', (data) => {
          setUserOnlineStatus(prev => ({ ...prev, [data.userId]: 'online' }));
        });

        socketRef.current.on('user-offline', (data) => {
          setUserOnlineStatus(prev => ({ ...prev, [data.userId]: 'offline' }));
        });
      };
  ...
  const loadMessages = useCallback(async (chatId) => {
    try {
      ...
      if (res.ok) {
        setMessages(data.messages || []);
        fetch('/api/chat/mark-read', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ chatId }) }).catch(e=>console.error('mark read failed', e));
      }
  ...
  const sendMessage = useCallback(async (e) => {
    ...
    const messageText = newMessage.trim();
    const tempId = 'temp-' + Date.now();
    const tempMessage = { _id: tempId, senderId: session.user.id, senderName: session.user.firstName || 'Admin', senderRole: 'admin', message: messageText, timestamp: new Date(), status: 'sending' };
    ...
    try {
      const res = await fetch('/api/chat/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chatId: selectedChat._id, message: messageText })
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const selectChat = useCallback((chat) => {
    ...
    loadMessages(chat._id);
    setOtherUserTyping(false);
    setChatUnreadCounts(prev => ({ ...prev, [chat._id]: 0 }));
    const totalUnread = Object.values(chatUnreadCounts).reduce((s,c)=>s+c,0) - (chatUnreadCounts[chat._id]||0);
    if (totalUnread === 0) window.dispatchEvent(new CustomEvent('clearAdminCount'));
    if (socketRef.current?.connected) socketRef.current.emit('join-chat', chat._id);
  }, [loadMessages, chatUnreadCounts]);

  const handleTyping = useCallback((value) => {
    setNewMessage(value);
    if (selectedChat && socketRef.current && session?.user) {
      socketRef.current.emit('typing', { chatId: selectedChat._id, userId: session.user.id, userName: session.user.firstName || 'Admin', isTyping: true });
      clearTimeout(typingTimeoutRef.current);
      typingTimeoutRef.current = setTimeout(() => {
        socketRef.current.emit('typing', { chatId: selectedChat._id, userId: session.user.id, userName: session.user.firstName || 'Admin', isTyping: false });
      }, 800);
    }
