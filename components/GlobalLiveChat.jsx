...
export function LiveChatProvider({ children }) {
  ...
  const showChat = useCallback(() => { setIsVisible(true); setUnreadCount(0); }, []);
  const hideChat = useCallback(() => setIsVisible(false), []);
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      // Emit user connection info for status tracking
      ...
      if (session.user.role === 'admin') socket.emit('join-admin');
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (reason === 'io server disconnect') {
        ...
        reconnectTimeoutRef.current = setTimeout(() => {
          socket.connect();
          // Re-emit connection info on reconnect
          setTimeout(() => {
            socket.emit('user-connect', {
              userId: session.user.id,
              role: session.user.role,
    ...
    socket.on('user-typing', () => { /* handled in component listeners */ });
    socket.on('admin-online', () => { /* handled in component listeners */ });
    socket.on('admin-offline', () => { /* handled in component listeners */ });
    socket.on('user-online', () => { /* handled in component listeners */ });
    socket.on('user-offline', () => { /* handled in component listeners */ });

    const handleClearAdminCount = () => { if (session.user.role === 'admin') setAdminUnreadCount(0); };
    window.addEventListener('clearAdminCount', handleClearAdminCount);
  ...
  useEffect(() => {
    if (chatId && socketInstance?.connected) socketInstance.emit('join-chat', chatId);
  }, [chatId, socketInstance]);

  return (
    <LiveChatContext.Provider value={{
      showChat, hideChat, isVisible, unreadCount, setUnreadCount,
      messages, setMessages, chatId, setChatId,
      adminUnreadCount, setAdminUnreadCount, socket: socketInstance
    }}>
      {children}
      {session && <LiveChat />}
    </LiveChatContext.Provider>
