...
export default function LiveChat() {
  ...
  const { data: session } = useSession();
  const {
    unreadCount, setUnreadCount, messages, setMessages, chatId, setChatId,
    adminUnreadCount, setAdminUnreadCount, socket
  } = useLiveChat();
  ...
  useEffect(() => {
    ...
    const handleTyping = (data) => {
      if (chatId && data.chatId === chatId && data.userId !== session?.user?.id) {
        setOtherUserTyping(data.isTyping ? { userName: data.userName || 'Support' } : null);
      }
    };
    const handleUserOnline = (data) => {
      if (data.role === 'admin') setOnlineStatus('online');
    };
    const handleUserOffline = (data) => {
      if (data.role === 'admin') setOnlineStatus('offline');
    };
    socket.on('user-typing', handleTyping);
  ...
  const handleTyping = useCallback((value) => {
    ...
    if (chatId && socket && session?.user) {
      if (!isTyping) {
        setIsTyping(true);
        socket.emit('typing', { chatId, userId: session.user.id, userName: session.user.firstName || 'User', isTyping: true });
      }
      clearTimeout(typingTimeoutRef.current);
      typingTimeoutRef.current = setTimeout(() => {
        setIsTyping(false);
        socket.emit('typing', { chatId, userId: session.user.id, userName: session.user.firstName || 'User', isTyping: false });
      }, 800);
    }
  }, [chatId, socket, session?.user, isTyping]);
  ...
  const handleOpen = useCallback(() => {
    ...
    setChatState('normal');
    if (!chatId) startChat(); else { loadMessages(chatId); setTimeout(()=>markAsRead(),100); }
  }, [session?.user?.role, chatId, startChat, loadMessages, markAsRead]);

  const getMessageStatus = (msg) => {
    if (msg.senderId !== session.user.id) return null;
    if (msg.status === 'sending') return 'sending';
    const adminRead = msg.readBy?.some(read => read.userId && read.userId !== session.user.id);
    return adminRead ? 'read' : 'delivered';
  };

  const getChatDimensions = () => {
    switch (chatState) {
      case 'maximized': return 'fixed inset-0 w-full h-full';
      case 'normal': return 'fixed bottom-6 right-6 w-80 h-96';
      case 'minimized': return 'fixed bottom-6 right-6 w-80 h-12';
  ...
  return (
    <>
      {chatState === 'collapsed' && (
        <div className="fixed bottom-6 right-6 z-50">
          <button onClick={handleOpen} className="relative bg-[#FE9900] hover:bg-[#F8C400] text-white p-4 rounded-full shadow-lg transition-colors">
            <FaComments className="text-xl" />
            {displayCount > 0 && (
              <div className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold">
                {displayCount > 9 ? '9+' : displayCount}
          ...
          <div className={`bg-gradient-to-r from-[#081C3C] to-[#0D274D] text-white p-4 ${chatState === 'maximized' ? 'rounded-none' : 'rounded-t-2xl'} flex items-center justify-between`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold">Lemufex Support</h3>
                <div className="flex items-center gap-1">
                  <div className={`w-2 h-2 rounded-full ${onlineStatus === 'online' ? 'bg-green-400' : 'bg-gray-400'}`}></div>
                  <p className="text-xs text-gray-300">{onlineStatus === 'online' ? 'Online now' : 'Offline'}</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={()=>setChatState(chatState==='maximized'?'normal':'maximized')} className="text-gray-300 hover:text-white transition-colors p-1">{chatState==='maximized'?<FaCompress/>:<FaExpand/>}</button>
              <button onClick={()=>setChatState('minimized')} className="text-gray-300 hover:text-white transition-colors p-1"><FaMinus/></button>
              <button onClick={()=>setChatState('collapsed')} className="text-gray-300 hover:text-white transition-colors p-1"><FaTimes/></button>
            </div>
          </div>
            ...
            <>
              <div ref={messagesContainerRef} className="flex-1 p-4 overflow-y-auto bg-[#F8F9FC] scroll-smooth">
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {messages.map((msg, index) => (
                      <div key={msg._id || index} className={`flex ${msg.senderId === session.user.id ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs px-3 py-2 rounded-lg text-sm ${msg.senderId === session.user.id ? 'bg-[#FE9900] text-white' : 'bg-white text-[#444] border border-gray-200'}`}>
                          <p>{msg.message}</p>
                          <div className={`flex items-center justify-between mt-1 ${msg.senderId === session.user.id ? 'text-orange-100' : 'text-[#A0AEC0]'}`}>
                            <p className="text-xs">
                              {msg.timestamp && !isNaN(new Date(msg.timestamp)) ? (()=>{
                                const msgDate = new Date(msg.timestamp);
                                const today = new Date();
                                const yesterday = new Date(today); yesterday.setDate(yesterday.getDate()-1);
                                if (msgDate.toDateString() === today.toDateString()) return msgDate.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                                if (msgDate.toDateString() === yesterday.toDateString()) return 'Yesterday ' + msgDate.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                                return msgDate.toLocaleDateString([], {month:'short', day:'numeric'}) + ' ' + msgDate.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                              })() : 'Now'}
                            </p>
                            {msg.senderId === session.user.id && (
                              <div className="ml-2">
                                {(() => {
                                  const status = getMessageStatus(msg);
                                  if (status === 'sending') return <div className="w-3 h-3 border border-current border-t-transparent rounded-full animate-spin opacity-50" />;
                                  if (status === 'read') return <FaCheckDouble className="text-xs text-blue-300" />;
                                  return <FaCheck className="text-xs" />;
                                })()}
                              </div>
                            )}
                    ...
                    {otherUserTyping && (
                      <div className="flex justify-start">
                        <div className="bg-gradient-to-r from-blue-50 to-blue-100 text-[#444] border border-blue-200 px-3 py-2 rounded-lg text-sm shadow-sm">
                          <div className="flex items-center gap-2">
                            <div className="flex gap-1">
                            ...
                            <span className="text-xs text-blue-600 font-medium">{otherUserTyping.userName} is typing...</span>
                          </div>
                        </div>
                      </div>
                    )}
