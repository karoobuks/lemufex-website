  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-8 border border-gray-100">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-[#002B5B]">Personal Information</h2>
        <button 
          onClick={() => router.push(`/edit/${userId}/edit-profile`)} 
          className="inline-flex items-center justify-center gap-2 bg-[#FE9900] text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-[#F8C400] transition-colors font-medium text-sm sm:text-base w-full sm:w-auto flex-shrink-0"
        >
        ...
        {infoItems.map((item, index) => (
          <div key={index} className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 bg-[#F8F9FC] rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[#444] font-medium text-sm sm:text-base break-words">{item.value}</p>
            </div>
          </div>
        ))}
      </div>
